...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="debug +ico +iso +wineappdb doas-as-sudo"
...
pkg_postinst() {
	einfo "Creating a sudo symlink to doas in /usr/local/bin..."
	ln -sv ../../../usr/bin/doas /usr/local/bin/sudo
}

pkg_postrm() {
	einfo "Removing the sudo symlink from /usr/local/bin..."
	rm -vf /usr/local/bin/sudo
}
